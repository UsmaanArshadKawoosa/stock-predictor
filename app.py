...
def predict():
    ...
    plt.plot(df.index[-len(y_test):], 
             scaler.inverse_transform(np.concatenate([np.zeros((len(y_test), 3)), y_test.reshape(-1, 1), np.zeros((len(y_test), 1))], axis=1))[:, 3], 
             label="Actual (Test Data)")
    plt.plot(df.index[-len(y_test):], 
             scaler.inverse_transform(np.concatenate([np.zeros((len(y_test), 3)), model.predict(X_test).reshape(-1, 1), np.zeros((len(y_test), 1))], axis=1))[:, 3], 
             label="Predicted (Test Data)", alpha=0.7)
    plt.plot(pd.date_range(df.index[-1], periods=289, freq="5min")[1:], predicted_future, 
             label="Predicted Next 24h", linestyle="dashed")
    plt.xlabel("Time")
    plt.ylabel("Price")
